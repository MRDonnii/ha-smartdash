(function () {
  ...
  function renderSettingsView() {
    ...
    const powerProfile = BeastLocalSettings.get("powerProfile", "balanced");
    return `
      <section class="admin-view${activeView === "settings" ? " is-active" : ""}" data-admin-view="settings">
        ...
        <div class="admin-card admin-settings-group"><div class="admin-card-head"><div><h2>${t("Strømprofil", "Power profile")}</h2><p>${t("Gælder kun denne kiosk. Lav begrænser forsiden til to samtidige kameraer og fjerner animationer; dine gemte kameravalg ændres ikke.", "Applies only to this kiosk. Low limits the overview to two simultaneous cameras and removes animations; saved camera choices are unchanged.")}</p></div></div><label class="admin-field"><span>${t("Profil", "Profile")}</span><select id="adminPowerProfile"><option value="performance"${powerProfile === "performance" ? " selected" : ""}>${t("Ydelse", "Performance")}</option><option value="balanced"${powerProfile === "balanced" ? " selected" : ""}>${t("Balanceret", "Balanced")}</option><option value="low"${powerProfile === "low" ? " selected" : ""}>${t("Lavt forbrug", "Low power")}</option></select></label><div class="admin-actions"><button type="button" class="admin-save" id="adminSavePowerProfile">${t("Gem strømprofil", "Save power profile")}</button><span class="admin-save-state" data-save-state="powerProfile"></span></div></div>
  ...
  function renderSecurityView() {
    ...
    const legacyLockAlarm = BeastConfig.get("screenLock.alarmEntity") || BeastConfig.get("panels.security.primaryAlarm");
    const configuredLockAlarms = BeastConfig.get("screenLock.alarmEntities");
    const selectedLockAlarms = Array.isArray(configuredLockAlarms) && configuredLockAlarms.length ? configuredLockAlarms : (legacyLockAlarm ? [legacyLockAlarm] : []);
    const alarmCandidates = BeastEntityPicker.candidates({ domain:"alarm_control_panel" });
    const alarmChoices = alarmCandidates.length ? `<div class="admin-checkbox-grid" id="adminLockAlarmEntities">${alarmCandidates.map((alarm) => `<label><input type="checkbox" value="${escapeHtml(alarm.id)}"${selectedLockAlarms.includes(alarm.id) ? " checked" : ""}><span>${escapeHtml(alarm.name)}</span></label>`).join("")}</div>` : `<p class="admin-empty">${t("Ingen alarm-enheder fundet.", "No alarm entities found.")}</p>`;
    ...
    return `<section class="admin-view${activeView === "security-settings" ? " is-active" : ""}" data-admin-view="security-settings">
      ...
      </div><div class="admin-security-alarm-rule"><label class="admin-field"><span>${t("Alarmer der låser kiosken", "Alarms that lock the kiosk")}</span>${alarmChoices}<small>${t("Hvis blot én valgt alarm er fuldt tilkoblet, låses kiosken. Den frigives først automatisk, når alle valgte alarmer er frakoblet.", "If any selected alarm is fully armed, the kiosk locks. It is only released automatically when all selected alarms are disarmed.")}</small></label><label class="admin-field"><span>${t("Når alle alarmer er frakoblet", "When all alarms are disarmed")}</span><select id="adminAlarmUnlockMode"><option value="pin"${alarmUnlockMode === "pin" ? " selected" : ""}>${t("Behold låsen · kræv PIN ved første brug", "Keep lock · require PIN on first use")}</option><option value="disarm"${alarmUnlockMode === "disarm" ? " selected" : ""}>${t("Fjern låsen · følg normal presence-styring", "Remove lock · resume presence control")}</option></select></label><div><strong>${t("Any armed", "Any armed")}</strong><small>${t("Dashboardet låses, og den valgte kioskskærm kan slukkes bagefter.", "The dashboard locks and the selected kiosk display can switch off afterwards.")}</small><button type="button" class="beast-security-action-btn" id="adminSaveAlarmLockRule">${t("Gem alarmregel", "Save alarm rule")}</button></div></div>${hasPin ? `<div class="admin-security-recovery"><div><strong>${t("Glemt din pinkode?", "Forgot your PIN?")}</strong><p>${t("Bekræft din identitet med et nyt Home Assistant-login, og opret derefter en ny kode.", "Confirm your identity with a new Home Assistant login, then set a new code.")}</p></div><button type="button" class="beast-security-action-btn" id="adminPinRecover">${t("Nulstil med HA-login", "Reset with HA login")}</button></div>` : ""}</div>
  ...
  function wireUi() {
    ...
    document.getElementById("adminSavePowerProfile")?.addEventListener("click", (event) => save(event.currentTarget, "powerProfile", () => BeastLocalSettings.set("powerProfile", document.getElementById("adminPowerProfile")?.value || "balanced")));
    ...
    document.getElementById("adminSaveAlarmLockRule")?.addEventListener("click", async (event) => {
      ...
      await BeastConfig.set("screenLock", {
        ...
        alarmEntity: null,
        alarmEntities: Array.from(document.querySelectorAll("#adminLockAlarmEntities input:checked")).map((input) => input.value),
